create proc dblog.usp_extract_job_status
as
declare @job_id             UNIQUEIDENTIFIER ,  
  ...
  @schedule_id        INT                 -- if supplied only return the jobs that use this schedule  
...
BEGIN  
  ...
  truncate table DBLOG.Current_Job_Status;
  insert  DBLOG.Current_Job_Status
  SELECT sjv.job_id,  
         originating_server,   
         sjv.name,  
         sjv.enabled,  
         ...
         fj.last_run_time,  
         fj.last_run_outcome,  
                       -- This column will be NULL if the job is non-local  
  ...
  /*
	To insert missing job entries
*/
insert into DBLOG.Current_Job_Status_report
(
Job_id,
originating_server,
job_name,
...
a.has_step,
a.has_schedule
from DBLog.Current_Job_Status a with (NOLOCK)
left outer join DBLOG.Current_Job_Status_report b with (NOLOCK)
...
*/
update a
set a.originating_server = b.originating_server,
a.job_name = b.job_name,
a.job_enabled = b.job_enabled,
a.job_description = b.job_description
, a.last_run_date=b.last_run_date
, a.last_run_time = b.last_run_time
, a.last_run_outcome = b.last_run_outcome
, a.has_step = b.has_step
, a.has_schedule = b.has_schedule

from DBLOG.Current_Job_Status_report a
join DBLog.Current_Job_Status b
	on a.job_id = b.job_id

/*
	Delete non-existing job entries
*/
delete a 
	from DBLOG.Current_Job_Status_report as a
left outer join DBLOG.Current_Job_Status as b
	on a.job_id = b.job_id
where b.job_id is null
END  