...
IF NOT EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = N'DBLog_Backup_Transfer_Differential')
BEGIN
DECLARE @jobId BINARY(16)
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBLog_Backup_Transfer_Differential', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'This job will transfer th differential backups to DR.', 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Transfer', 
		...
		@command=N'EXEC dblog.usp_Backup_Transfer_Files_DiffOnly', 
