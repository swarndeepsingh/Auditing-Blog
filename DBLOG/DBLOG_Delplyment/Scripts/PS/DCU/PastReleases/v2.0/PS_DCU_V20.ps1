...
function SDS-create_RTM_Inventory ()
{
   ...
   $servernameverify=
   {
    ...
    insert into [RTM].[RTM].[ServersList] (IPAddress, [Name], Active, AutoPoll, ServerType, LicenseType, ParentLicense, DateTimeAdded, AddedBy, ApplicationID, environmentid, Datacenterid)
                                    values ('$ipaddress', '$ServerName', '1', '1', 'server', '$licensetype', '$parentlicensedserver', getdate(), 'DCU', '$applicationid', '$environmentid', '$datacenterid')" 
}
