...
### If the SQL Server where this is being run
### does not have SQL Server 2014 or SQL Server 2014 Management Studio
### Then download and install SQLSysClrTypes.msi, SharedManagementObjects.msi and PowerShellTools.msi 
### from this link: 
### --------------------------------------------------------------
### https://www.microsoft.com/en-us/download/details.aspx?id=42295
### --------------------------------------------------------------
### Also you may need Powershell 4.0, which is in
...
if ($script:sqlserver -ne "")
{
    if ($script:rootfolderitems -ne "")
    {
            ...
            foreach ($bkupfile in Get-ChildItem  $script:rootfolderitems | Select-Object basename)
            {
                ...
                $backupfilepath = $script:rootfolderitems + $bkupfile.BaseName + '.dmp'
