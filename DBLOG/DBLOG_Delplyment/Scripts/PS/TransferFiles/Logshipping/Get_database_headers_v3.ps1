...
Add-Type -Assembly 'Microsoft.SqlServer.BatchParser, Version=10.0.0.0, Culture=neutral, PublicKeyToken=89845dcd8080cc91'
...
{  
    ...
    {
        get-Event "Comparing $folder\*.$backupext for $drservername with existing data in table"
   ...
   if ($script:backuptool -eq "SQL")
    {
        ...
        foreach ($bkupfile in $filestoberead )
        {
                ...
                $backupfilepath = "$folder\$($bkupfile.Inputobject)"
                ...
                #try
                #{
                    #$header = $restore.ReadBackupHeader([Microsoft.SqlServer.Management.Smo.Server]$server)
                    
                    $header=invoke-sqlcmd -ServerInstance $script:DRSERVER -Database "master" -query "restore headeronly from disk = '$backupfilepath'"
                ...
                #}
                        ...
                        get-Event  "Writting header information from $($bkupfile.inputobject) to database"
                         ...
                         $insertquery = "INSERT INTO [DBLOGDR].[dbo].[BackupHeaders]
                           ...
                           select 
                           '$($bkupfile.inputobject)',           -- filename
                           '$($header.DatabaseName)', -- DBNAME
                           '$($header.ServerName)', --SQLServer
                           '$($header.FirstLSN)', --first lsn
                           '$($header.LastLSN)', --last lsn
                           '$($header.CheckpointLSN)', --checkpointlsn
                           '$($header.DatabaseBackupLSN)', --databasebackuplsn
                           
                            '$($header.BackupSize)', --nativedbsize
                           '$($header.BackupType)', --backuptype
                            '$($script:backuptool)' ,                --toolid
                            '$folder',
                            '$($header.BackupStartDate)', --Backup Start Time
                            '$($header.BackupFinishDate)' -- Backup End Time
                           "
                            try
                            {
                                # get-Event $insertquery
                                if ($header.DatabaseName -ne "nothing" -or $header.DatabaseName -eq $null -or $header.DatabaseName -eq '' )
                                {
                                    Invoke-SQLCMD -Query "$insertquery" -ServerInstance $script:drserver  -Database "DBLOGDR" -verbose
                                }
                            }
