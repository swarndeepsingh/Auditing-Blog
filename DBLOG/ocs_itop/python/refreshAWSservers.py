...
class iTopOperations:
	...
	def getNewAWSHosts():
		...
		func="getNewAWSHosts" # Enter function name here
		...
		for row in gd.load_from_db(qr.query()['AWSServers']):
			
			# get list from iTop that matches the row mentioned above
			uri=(env+'json_data={"operation": "core/get", "class": "OSFamily", "key":"SELECT OSFamily WHERE name=\'' + row['name'] + '\'", "output_fields": "id, friendlyname"}')
...
iTopOperations.getNewOSFamilies()
