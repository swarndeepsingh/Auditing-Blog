	...
	def setlocations():
		...
		if (data['code']!=0):
			...
			for k,v in data.get('objects').items():
				mylist.append(data.get('objects').get(k).get('fields').get('friendlyname'))
			
			#list of items in OCS but not in iTop
			for y in [x for x in rows if x['idc'] not in mylist[0:len(mylist)]]:
				try:
					uri_obj=(env+'json_data={"operation":"core/create", "comment":"Created by itop synch process: swarndeep", "class":"Location", "fields" : {"name":"' + y['idc'] + '","org_id":"SELECT Organization WHERE name=\'EBIX\'"}}')
					res=requests.post(uri_obj)
					jdata=json.loads(res.text)
					if(jdata['code']==0):
						msg="Object Created: {}".format(res.text)
						logs.writelog(func, msg)
					else:
